    ...
    <div class="container">
      <div aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active" aria-current="page">
            <a href="{{ url('/monitors') }}">Màn hình máy tính </a>
          </li>
        </ol>
      </div>
      ...
      <div class="p-container bg-white js-box-container" style="min-height: 400px" data-id="395-850">
        <div class="container custom-nav owl-carousel owl-theme">
          @foreach ($topMonitors as $monitor)
            @include('partials.simple-p-item', ['product' => $monitor])
          @endforeach
        </div>
      </div>
      ...
      <div class="product-collection-related-container">
        ...
        <div class="paging bg-white mx-auto">
          <a href="{{ $monitors->url(1) }}" class="{{ $monitors->onFirstPage() ? 'current' : '' }}">
            1
          </a>
          @for ($page = 2; $page <= $monitors->lastPage(); $page++)
            <a href="{{ $monitors->url($page) }}"
              class="{{ $page == $monitors->currentPage() ? 'current' : '' }}">
              {{ $page }}
            </a>
          @endfor
          @if ($monitors->hasMorePages())
            <a href="{{ $monitors->nextPageUrl() }}">
