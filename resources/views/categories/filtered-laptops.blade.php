    ...
    <div class="container">
      ...
      <div class="product-collection-related-container">
        <div class="group-title">
          <h2 class="title">KẾT QUẢ TÌM KIẾM LAPTOP</h2>
        </div>
      </div>
      ...
      <div class="product-collection-related-container">
        ...
        <div class="filter-container">
          ...
          <div class="filter-item filter-selected-list">
            <p class="group-title"> Đã chọn: </p>
            <div class="d-flex flex-wrap">
               @foreach ($filters as $filterName => $filterValue)
                    @if (!empty($filterValue))
                        <a href="#" class="selected-filter">
                            {{ ucfirst($filterValue) }}
                        </a>
                    @endif
                 @endforeach
                <a href="{{route('gaming-laptops.show')}}" class="btn-delete"> Xoá tất cả </a>
            </div>
          </div>
        </div>
        ...
        <div class="p-container bg-white">
          <div class="sort-container d-flex align-items-center justify-content-between flex-wrap">
            <div class="d-flex align-items-center">
              <b id="product-count-display" style="font-size:16px;margin-right: 20px"> Laptop</b>
              <div class="sort-by-group">
                <a href="/laptops/sort?sort=asc">
                  <i class="mb-icons icon-increase" data-filter_code = "sort" data-value="asc"></i>
                  <span> Giá tăng dần </span>
                </a>
                <a href="/laptops/sort?sort=desc">
          ...
          <div class="p-list-container d-flex flex-wrap">
            @if ($laptops->isEmpty())
                <p class="no-products">Không có sản phẩm cần tìm.</p>
            @else
        ...
        <div class="paging bg-white mx-auto">
          {{-- Trang đầu tiên --}}
          <a href="{{ $laptops->url(1) }}" class="{{ $laptops->onFirstPage() ? 'current' : '' }}">
            1
          </a>
          {{-- Các trang tiếp theo --}}
          @for ($page = 2; $page <= $laptops->lastPage(); $page++)
            <a href="{{ $laptops->url($page) }}"
              class="{{ $page == $laptops->currentPage() ? 'current' : '' }}">
              {{ $page }}
            </a>
          @endfor
          {{-- Nút chuyển sang trang tiếp theo --}}
          @if ($laptops->hasMorePages())
            <a href="{{ $laptops->nextPageUrl() }}">
...
@push('scripts')
      ...
      element.addEventListener('click', function(e) {
        ...
        if (selectedFilterType === 'price' && selectedMin !== null && selectedMax !== null) {
          window.location.href =
            `/laptops/${cat}/filter?cat=${cat}&min=${encodeURIComponent(selectedMin)}&max=${encodeURIComponent(selectedMax)}`;
        } 
        else {
          window.location.href =
            `/laptops/${cat}/filter?cat=${cat}&${selectedFilterType}=${encodeURIComponent(selectedValue)}`;
        }
      });
    });
});
  document.addEventListener('DOMContentLoaded', function() {
    ...
    productCountDisplay.textContent = `${productCount} Laptop`;
  });
  </script>
@endpush
