               ...
               @foreach ($filters as $filterName => $filterValue)
                    @if (!empty($filterValue))
                        <a href="#" class="selected-filter">
                            {{ ucfirst($filterValue) }}
                            <i class="fas fa-times" data-href="{{ route('gaming-laptops.show') }}"></i>
                        </a>
                    @endif
                 @endforeach
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
      document.querySelectorAll('.selected-filter i').forEach(function (element) {
          element.addEventListener('click', function (e) {
              e.preventDefault();
              const route = element.getAttribute('data-href');
              window.location.href = route; // Điều hướng về route gaming-laptops.show
          });
      });
  });
</script>
